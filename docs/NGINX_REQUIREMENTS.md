...
Трафик от пользователей поступает на NGINX, который проксирует его
к приложению Spring Boot. Приложение обращается к базе PostgreSQL.
Метрики NGINX экспортируются отдельным процессом.

### 2.2 Режим развёртывания
NGINX запускается как отдельный сервис перед приложением, выполняя роль
...
Для обновления конфигурации и приложения применяется стандартный rolling update.
