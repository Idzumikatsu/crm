...
2. Поместите действующий сертификат и ключ `server.crt` и `server.key`
   в каталог `infra/nginx/certs`. В CI/CD эти файлы можно формировать
   из секретов `TLS_CERT` и `TLS_KEY`.
...
После первого запуска при активном профиле `dev` (или напрямую `h2`) создаются учетные записи менеджера, преподавателя и администратора.
Параметры входа (только для разработки):
- менеджер: `manager`/`manager`
- преподаватель: `teacher`/`teacher`
- администратор: `admin`/`admin`

В продукционном профиле `postgres` и в профиле `test` эти пользователи не создаются, что упрощает написание интеграционных тестов. Эти учётные записи не предназначены для production.
...
Команда `npm run dev` запускает Vite и Tailwind в watch‑режиме,
Этот режим предназначен только для разработки, поэтому стили пересобираются при каждом сохранении исходников.
Новый компонент `<Layout>` обеспечивает боковую навигацию,
а устаревшие статические HTML‑страницы удалены из каталога
