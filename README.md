...
3. Создайте сертификат для Nginx. Выполните команду из корня репозитория,
  ...
  Затем сохраните содержимое файлов в секретах `SSL_CERT`, `SSL_CA_CERT` и `SSL_KEY`.
...
По умолчанию запросы отправляются относительным путём, но в production имеет смысл указать полный адрес, например `VITE_API_URL=https://crm-synergy.ru`.
...
- `SSL_CERT`, `SSL_CA_CERT` и `SSL_KEY` – содержимое сертификата, цепочки и закрытого ключа для NGINX. Workflow распознает PEM и Base64 форматы и при необходимости выполняет декодирование.
