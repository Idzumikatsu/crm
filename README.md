...
умолчанию `80` и `443`). При необходимости задайте другие значения в файле
...
По умолчанию запросы отправляются относительным путём. В production переменная должна быть установлена в `/api`, потому что обращения к бэкенду проходят через Nginx.
