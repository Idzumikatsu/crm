...
5. **PostgreSQL**
   ...
   - В `application.yml` адрес БД задаётся через переменную окружения `DB_HOST`.
     По умолчанию используется `db` (можно переопределить).
   - Для разработки и продакшена используется PostgreSQL,
...
Если переменные не заданы, отправка сообщений будет пропущена, а попытки
зафиксированы в логе. Проверка почтового сервера отключена свойством
